...
def send_information_about_update_periodical():
    ...
    subscriptions = Subscription.objects.filter(is_active=True, ).select_related("course", "user",)
    ...
    for subscription in subscriptions:
        ...
        if (
                subscription.last_seen_update_at is None or
                subscription.last_seen_update_at < course.updated_at
        ):
